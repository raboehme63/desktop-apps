...
- **Reiseabschnitte** legt der Benutzer in der Timeline an: aus einer Mehrfachauswahl (Aufenthalt oder Transfer) oder ohne Auswahl als leeren Abschnitt (Tag mit **Am**, Aufenthalt/Transfer mit **Von Datum** / **Bis Datum**). ⋯ **Datum…** bzw. **Zeitraum…** ändert die Spanne später; die Karte rutscht in der Timeline an die passende Stelle. Dateien ohne Abschnitt bleiben **Tage**. Der Typ (Tag, Transfer, Aufenthalt) ist an jeder Timeline-Karte änderbar. **Löschen** im ⋯-Menü legt alle Medien in den Medienpool. Neu angelegte Abschnitte existieren nur im Speicher, bis Timeline-**Speichern**. **Zur Karte** an einem gespeicherten Abschnitt oder Tag öffnet die Karte und fokussiert die passende Karte in der Leiste.
