...
| Status | Leitkonzept; Phase 7, Software **R2.1.0** |
...
### 3.3 Bearbeitung (Phase 7, Software R2.1.0)
...
| **Karte** | Geografische Wahrheit: ein runder Kreis je Tag, Transfer oder Aufenthalt; zwischen Tag- und Aufenthaltskreisen Verbindungslinien in Timeline-Reihenfolge (Transfer-Liste oder Ausgangslinie, Symbolspitze zum Folgekreis, ausgeblendet, wenn sich die Kreise überdecken). Der Transfer-Kreis hängt mit einer dünnen Linie am Verkehrssymbol. Klick auf Kreis oder Symbol zeigt Fotos, Videos, Tracks und Orte dieses Eintrags. Die Leiste unter der Karte folgt dem Reiseverlauf — Einfachklick zentriert bei gleichem Zoom, Doppelklick öffnet die Timeline. Ohne Eintrags-Titelbild das erste Foto, sonst das erste Track-Thumbnail, sonst das erste YouTube-Vorschaubild. Abschnitte ohne Position haben in der Leiste einen roten Rand; Rechtsklick **Platzieren**, **Verschieben** (Fadenkreuz, Zoom und Fokus bleiben) oder **Zentrieren** (schwenkt und zoomt auf den Kreis). Backend austauschbar. |
...
Aktueller Konzeptstand: **Phase 7**, Software **R2.1.0** (Journal-Modell nach Design-Review plus Verbindungslinien: Transfer-Liste und Ausgangslinie, Verkehrssymbole in Fahrtrichtung, Satelliten-Overlays; Timeline, Bewertungen, Inspektor, Track-Vorschauen, Karten-Leiste, Kreis-Detail, Rückgängig/Wiederherstellen). Windows-Endnutzerpaket (onedir/Zip, optional Inno-Setup) ist baubar und keine eigene Fachphase. HTML-Export folgt in Phase 8.
