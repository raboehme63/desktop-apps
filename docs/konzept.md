...
Der Benutzer sieht während des Imports Fortschritt und danach **die vollständige Dateiliste** mit Aufnahmezeit, GPS und Kamera. Klick oder Mouseover auf eine Zeile zeigt das gecachte Vorschaubild und die gespeicherten Metadaten. Die Statistik (Fotos, Videos, MAP, Activity, Flüge, Sonstige, Texte, Fehler) und die Tabelle müssen dieselbe Grundmenge beschreiben. Fitness- und IGC-Datenbankpfade bleiben in `config.json` gemerkt. Nach dem Import entsteht automatisch die Timeline; Karte und Galerie lesen denselben Index.
