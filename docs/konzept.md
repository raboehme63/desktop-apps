...
| Status | Leitkonzept; Phase 7 plus Medien-Pipeline, Software **R3.4.0** |
...
Der Benutzer sieht während des Imports Fortschritt und danach **die vollständige Dateiliste** mit Aufnahmezeit, GPS und Kamera. Klick oder Mouseover auf eine Zeile zeigt das gecachte Vorschaubild und die gespeicherten Metadaten. Die Statistik (Fotos, Videos, MapTracks, ActivityTracks, Flüge, Sonstige, Texte, Fehler) und die Tabelle müssen dieselbe Grundmenge beschreiben. Der Activity-Datenbankpfad bleibt in `config.json` gemerkt. Nach dem Import entsteht automatisch die Timeline; Karte und Galerie lesen denselben Index.
...
Polar-Trainings-JSON mit `routes` importiert die App nicht. Dafür gibt es die eigenständige Fitness-Datenbank (`packages/fitnesscore`, CLI `python -m fitnesscore`; Aufruf und Parameter im [README](../README.md)). Auf der Seite **Import** holt man Activity-GPX und IGC aus **einer** Datenbank (Dialog für den Dateinamen, Standard `activity.sqlite`; **Laden…** mit Zeitraum aus Reise von–bis und Checkboxen **Activity-Tracks** / **Flüge**; **Neu laden** gleicht ab: neue dazu, fehlende aus den Ordnern, dem Index und den Karten entfernt; Altbestand `.FitnessTracks/` wird mitgeleert; die Zähler MapTracks/ActivityTracks/Flüge/Sonstige werden sofort angepasst) nach `{Import}/.ActivityTracks/` bzw. `{Import}/.IGCTracks/`. Der Datenbankpfad bleibt gemerkt. Scan und Synchronisieren nehmen beide Ordner mit (normale GPS-Dateien, im Gegensatz zu `.MapTracks/`). Das Hilfsskript `scripts/json_routes_to_gpx.py` schreibt weiterhin eine GPX-Datei neben die JSON-Datei (Aufruf und Kurzhilfe im [README](../README.md)). Google-Maps-Routenlinks werden auf der Ausgangslinie (Tag/Aufenthalt, Geometrie **Map-Track**) und am Transfer als Verbindungstyp **Map-Track** oder per `scripts/maps_url_to_gpx.py` zu GPX; die Datei liegt unter `.MapTracks/` im Import-Ordner. Aus einem Maps-Link schlägt die App einen Namen aus Anfangs- und Endpunkt vor (Dialog, überschreibbar; fehlt der Name im Link, Nominatim oder `Stop-n`). Ein Datei-Import bleibt **Map-Track**.

### 3.3 Bearbeitung (Phase 7 plus Medien-Pipeline, Software R3.4.0)
...
| --- | --- |
| **Projekt** | Behälter: Übersicht mit sichtbarem Suchfeld und ausgewähltem Projekt (zuletzt geöffnetes zuerst); einklappbare Projektliste (Standard ein, Klick wählt und klappt zu, eine Zeile: Reisetitel, Datum, Verzeichnis; Wildcards `*`/`?`; Sortierung Name/Datum; ab sechs Einträgen scrollbar), Name, Ordner, Öffnen (fragt bei Auswahl: ausgewähltes oder anderes Projekt; anderes öffnet den Ordnerdialog; standardmäßig Nur lesen)/Anlegen; **Reise von–bis** (vorbefüllt aus indexierten Daten, editierbar, inklusiv die Kennzahl Tage); **bereiste Länder** aus dem Katalog (ISO-2, Flagge, Umriss). Keine Medienbearbeitung. |
| **Import** | Brücke zur Außenwelt. Einzige Stelle, die das Quellverzeichnis scannt. Analyse ergänzt; Synchronisieren entfernt Fehlende und fragt Timeline oder Pool für Neue. Activity-DB nach `.ActivityTracks/` / `.IGCTracks/`; **Neu laden** räumt Ordner, Index und Karten. Zähler MapTracks/ActivityTracks/Flüge/Sonstige. Thumbnail-Schieber skaliert die rechte Dateivorschau. |
...
SQLite ist die Arbeitsdatei, nicht das Archiv der Bilder. Thumbnails und Analyseergebnisse liegen neben der Datenbank, damit das Projekt kopierbar bleibt. Die Quellwurzel der Originale steht in `settings.toml` (Projekt → Einstellungen), zusammen mit GPS-Zeitfenster, Standardzeitzone, Kartenanbieter (`leaflet` / `offline`), der Farbe der Verbindungslinien auf der Karte (Standard weiß), der Map-Track-Farbe (Standard blau) und dem Reise-Standard für Linientyp, Strich und Verkehrssymbol (Abschnitte können das überschreiben). Map-Tracks speichert die App als GPX unter `.MapTracks/` in diesem Import-Ordner (nicht im Projektordner); Scan und Synchronisieren überspringen den Ordner. Fitness-Tracks aus der Fitness-Datenbank liegen unter `.ActivityTracks/` im selben Import-Ordner (Altbestand `.FitnessTracks/` wird beim Scan und beim Neu-Laden noch erkannt), IGC-Tracks unter `.IGCTracks/`; Scan und Synchronisieren nehmen beide mit. Wird der Ordner verschoben, setzt man den neuen Pfad; der Index wird umgeschrieben, die Originaldateien nicht. Zuletzt geöffnete Projekte merkt die App unter `%LOCALAPPDATA%\TravelJournal\recent.json`. Die Projektseite zeigt den Stammordner und diese Recents als Übersicht; tote Pfade sind gekennzeichnet. Das Medienregister (`timeline_media_tab`, Timeline und Medienseite), die eingeklappten Timeline-Galerien (`timeline_galleries_collapsed`), die Thumbnail-Schieber (`timeline_thumb_zoom`, `map_thumb_zoom`, `media_thumb_zoom`, `import_thumb_zoom`), die eingeklappte Navigation (`sidebar_collapsed`), der Medienpool (`timeline_pool_visible`, `pool_width`), die Inspektor-Fenstergröße (`inspector_width`, `inspector_height`, `inspector_maximized`), Die Activity-Datenbank (`activity_db_path`) und der Projekte-Stammordner liegen in `config.json`. Der Fenstertitel zeigt die Softwareversion (`Reisetagebuch R3.3.0` bzw. mit Projekttitel). Die Kartendetail-Schalter **Flüge anzeigen** und **Aktivitäten anzeigen** stehen in `settings.toml` (`map_show_flights`, `map_show_activities`). `inspector_show_rejected` steuert die Inspektor-Checkbox **Aussortierte anzeigen**. Bereiste Länder (`trips.countries`, ISO-2) und Reise von–bis (`trips.start_date` / `trips.end_date`) gehören zur Projektdatenbank, nicht zu `settings.toml`.
...
- **Karte** (Travelbook interaktiv): die Reisekarte als **Website**, nur lesen. Schwenken, zoomen, Layer, Abschnitte und Fotos ansehen — wie die Kartenseite der App, ohne Editor (kein Platzieren, keine Bewertung, kein Tagebuchtext). Leiste ziehbar, Thumbnail-Schieber, Doppelklick öffnet die große Bildvorschau (kein Original). Nur HTML; lokal im Browser, ohne Server.
...
**Travelbook** (Phase 8, zuerst HTML) folgt der Buchmetapher. Auf der Seite Export stehen oben **Vorschau** und **Editiermodus**. Man wählt den **Ausgabetyp**, danach das **Seitenverhältnis einer Seite** (Standard DIN A4 Hochformat; auch Querformat und Quadratisch). Das Buch füllt den restlichen Arbeitsbereich und wird in die Fenstergröße eingepasst (Cover eine Seite, sonst Doppelseite). Blättern: Cover, Titelseite, Reiseübersicht, Chronik. Das **Ausgabeformat** wird erst beim Klick auf Exportieren gefragt, und nur die Formate des Typs. Die Komposition liegt als `travelbook.json` im Projektordner.

1. **Buchdeckel** — ein Vollbild, Titel zentriert, optionales Jahr-Badge. Die Blätterleiste zeigt „Cover“, nicht 1/n.
2. **Titelseite** — Doppelseite: links leer, rechts der Reisetitel mittig zentriert. Ohne Seitenzahl (Blätterleiste: „Titelseite“).
3. **Reiseübersicht** — erste nummerierte Doppelseite 1–2/n. Links zweispaltig: dunkle Länderleiste (Auswahl auf der **Projektseite**; jedes Land als Umriss, darunter Name in Versalien mit kleiner Flagge dahinter, gleichmäßig über die Seitenhöhe) und Kennzahlen (Tage, Reiseabschnitte, Fotos, YouTube-Videos, Gleitschirmflüge/IGC). Rechts ein **statisches** Satellitenbild mit Route und Foto-Markern. Die Nummerierung beginnt hier.
4. **Chronik** — ein Kapitel je veröffentlichtem Abschnitt (`published_entries`):

   - **Erste Doppelseite (automatisch):** links Abschnittsstart. Oben ein Block **Länderumriss mit ungefährer Position** (Karten-Pin, sonst GPS von Titelbild oder Medien) plus **Flagge und Land**; rechts daneben das **Titelbild im Bildformat** (Freiraum über/unter dem Block, wenn das Foto höher ist). Darunter **Abschnittsname**, Datum als **01.01.1900** bzw. **01.01.1900 bis 22.02.1900**, graue Tagebuchbox mit dem Timeline-Text, unten dieselbe Datumsangabe auf der Reise-Zeitleiste. Rechts die Fotos des Abschnitts (Template photos_1–8 nach Anzahl, höchstens acht). Die linke Seite ist gesperrt, die rechte Layoutwahl und die Fotos sind editierbar.
...
**Medienleiste (Editiermodus):** Zum geöffneten Abschnitt stehen die sichtbaren Mitglieder bereit — Fotos, Videos und Tracks als Thumbnails (dieselben Caches wie Timeline/Galerie; Aussortierte und versteckte Cluster-Mitglieder fehlen, Pool nicht). Per **Drag-and-drop** auf die Media-Slots der aktuellen Foto-Seiten. Ein Slot zeigt das Vorschaubild; Tracks nutzen die Track-Vorschau. Dieselbe Datei darf mehrfach liegen. Ablegen auf einem belegten Slot ersetzt. Die Komposition schreibt `source_file_id` nach `travelbook.json`, nicht in die Timeline und nicht ins Original. Die Intro-Seite und die Journal-Seite haben keine Media-Slots.

Flagge und Länderumriss kommen aus dem lokalen Katalog (`travelcore.geo`, ISO-3166-1). Welches Land ein Abschnitt hat, folgt der Position (Karten-Pin, sonst GPS von Titelbild oder Medien) im **Umriss** des Katalogs (Abstand zum Polygon, nicht nur Bounding-Box — sonst würde z. B. Österreich Süddeutschland schlucken). Überlappen vereinfachte Grenzen, gewinnt das größere Land, in dem der Punkt liegt. Die bereisten Länder der Projektseite entscheiden nur bei gleich großen Treffern. Ohne Position und mit nur einem Reiseland gilt dieses Land ohne Markierung. Das Titelbild steht rechts neben dem Block Umriss/Flagge/Land im Bildformat.
...
**Travelbook (interaktiv)** ist kein Buch und erbt keine Buchseiten. Es exportiert die **Karte** (`MapScene` der veröffentlichten Abschnitte) als Read-only-Website: Schwenken, Zoomen, Layer, Cover-Klick, Foto-Popup, ziehbarer Leiste, Tagebuchtext, YouTube-Thumbs, Thumbnail-Schieber (50–200 %). Doppelklick auf das Popup öffnet eine Lightbox mit einem 1920-px-JPEG unter `media/full/` (Originale werden nicht kopiert). Leaflet liegt lokal im Paket; Basemap-Kacheln bleiben online. Ausgeblendete Abschnitte fehlen wie in der App. Redaktion bleibt in der App.
...
Aktueller Konzeptstand: **Phase 7** plus Medien-Pipeline, Software **R3.4.0** (Journal-Modell nach Design-Review plus Verbindungslinien; Sichtbarkeits-Schalter Karte/Export; **Speichern** grau außer bei Dirty-Stand, Undo/Redo schaltet mit, Verlassen-Dialog Speichern/Verwerfen/Abbrechen; Kartenbedienung: Cover-Zoom, Fit-Reise, vorab zentriertes Foto-Popup mit Blättern und Schieber-Zoom, Detail-Checkboxen **Flüge anzeigen** / **Aktivitäten anzeigen**; **Zur Karte** letzter Klick gewinnt, geladene Karte ohne Neuaufbau, Sichtbarkeitswechsel ohne Timeline-Neubau; Track-Bewertung wie Medien; Timeline mit **Menü**, Inspektor mit Schlüsselfotos, Track-Vorschauen (Leaflet), Chips **Map** / **Act** / **igc**, Karten-Leiste, Rückgängig/Wiederherstellen; Import: Fitness-/IGC-Karten mit gemerkter DB, Zähler MapTracks/ActivityTracks/Flüge/Sonstige, kanonisch `.ActivityTracks/` (**Neu laden** räumt Ordner, Index und Karten); Stapel/Gruppe/Statistik, Qualitätsampel mit Hover, **Filtern** und Import-Thumbnail-Schieber; Projektseite: Länderkatalog und Reise von–bis; **Travelbook (interaktiv)** als HTML-Ordner mit Leiste, Tagebuch, YouTube, Lightbox und Zoom-Schieber). Windows-Endnutzerpaket (onedir/Zip, optional Inno-Setup) ist baubar und keine eigene Fachphase. Buch-HTML folgt in Phase 8.
