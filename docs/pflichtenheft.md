...
| ID | Prio | Anforderung | Stand |
| --- | --- | --- | --- |
| FA-080 | Muss | Tagebuch vollständig bearbeitbar: Tage, Orte, GPS, Ereignisse, Texte, Fotos, Reihenfolge, Titelbilder, Abschnitte. | teilweise (Bearbeitung in der Timeline: Reisetitel, Tage, Titel/Text, Abschnitte, Eintrags-Titelbilder; Reisetitel, Titel, Texte und YouTube gemäß FA-064; `used_in_journal` und Reise-Titelbild `is_cover` in `travelcore`, ohne eigene UI; kein Umsortieren von Fotos auf andere Tage, keine Ereignis-Reihenfolge) |
