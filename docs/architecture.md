...
| `export` | Vertrag `Exporter`; Produkt- und Seiten-Templates JSON; Travelbook-Editiermodus (`travelbook.json`); Katalog Typ×Format; Raster-PDF (Travelbook); HTML-Renderer noch Platzhalter (erster Pfad: Travelbook × HTML) |
...
- `Exporter` – HTML, PDF, LaTeX, CEWE (Implementierung ab Phase 8). Ausgabetyp ist ein JSON-Template unter `travelcore/export/templates/products/`. **Travelbook** und **Jahrbuch** sind Blätterbücher (Karten als Bild). Travelbook hat einen Editiermodus (`travelbook.json` im Projekt): erste Doppelseite je Abschnitt, weitere Spreads mit Seiten-Layouts `photos_1`–`photos_8` und `journal`; Medienleiste des Abschnitts (Foto/Video/Track-Thumbs) per Drag-and-drop auf Media-Slots. **Travelbook (interaktiv)** ist die Read-only-Kartenwebsite (schwenken, zoomen), nur HTML. Katalog: `templates/catalog.json`. Erster Pfad: Travelbook × HTML. PDF: Raster-`PdfRenderer` (Pillow-Seiten, JPEG-PDF), kein PyMuPDF. `load_product` / `load_page_layout`; `supports(typ, format)` prüft die Matrix.
