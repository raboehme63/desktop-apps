...
| `export` | Vertrag `Exporter`; Produkt- und Seiten-Templates JSON; Travelbook-Editiermodus (`travelbook.json`); Katalog Typ×Format; Raster-PDF (Travelbook); CEWE-Projekt `.mcf` (Travelbook A4, editierbarer Hybrid); HTML-Renderer noch Platzhalter (erster Pfad: Travelbook × HTML) |
